...
CWiremanStage::CWiremanStage() :
	m_iScore(NULL), m_iMiss(3), m_bClear(false), m_iClearScore(10000)
{
}
...
void CWiremanStage::Render(HDC _hDC)
{
	...
	wstring wstr = L"남은 공 : " + to_wstring(m_iMiss);
	CFontMgr::Get_Instance()->Draw_MyFont(_hDC, ML + 50, MB - 20, 50, 20, wstr);

	wstr = L"점수 : " + to_wstring(m_iClearScore) + L" / " + to_wstring(m_iScore);
