...
void CSkiingPlayer::Render(HDC hDC)
{
	...
	wstring wstr = L"Time : " + to_wstring(m_iTime);
	CFontMgr::Get_Instance()->Draw_MyFont_Color(hDC, WINCX / 2, 100, 100, 100, wstr);

	wstring wstr2 = L"Score : " + to_wstring(m_iScore) + L" / 10";
	CFontMgr::Get_Instance()->Draw_MyFont_Color(hDC, 800, 100, 100, 100, wstr2);

	wstring wstr3 = L"Start : " + to_wstring(m_iStartCount);
	CFontMgr::Get_Instance()->Draw_MyFont_Color(hDC, 150, 100, 100, 100, wstr3);
...
void CSkiingPlayer::StateUpdate()
{
	...
	case JUMP:
	{
		if (m_bSound)
		{
			CSoundMgr::Get_Instance()->StopSound(SOUND_PLAYER2);
...
int CSkiingPlayer::InCollision(CObj* _target, D3DXVECTOR3 _dir, float _Angle)
{
	...
	case SKIING_FLAG:
	{
		...
		CSoundMgr::Get_Instance()->PlaySoundW(L"Score.mp3", SOUND_PLAYER, 10.f);
		if (m_State == JUMP)
		{
			m_iScore += 3;
		}
