...
void CTank::Render(HDC hDC)
{
	...
	swprintf_s(szBuff, L"Life : %d", m_iLife);
	TextOut(hDC, 700, 650, szBuff, lstrlen(szBuff));
	//if (g_CollideCheck)
...
{
	CSoundMgr::Get_Instance()->PlaySound(_name, SOUND_EFFECT1, SOUND_VOL2);
}
...
int CTank::InCollision(CObj* _target, D3DXVECTOR3 _dir, float _Angle)
{
	...
	{
		if (_target->GetOwner()->Get_Type() != TANK)
		{
